...
await cnf.setStatus(`Bot activo durante:${uptime}\nConfuBot5`).catch((_) => _);
...
if (isBanned) return  reply(mess.banned)
        if (!isGroup) return reply('esta función es solo para grupos');
if (!isGroupAdmins) return reply('solo un administrador puede usar este comando')
htg = args.join(" ")
					htgg = htg.split("|")[0];
					httg = htg.split("|")[1];
